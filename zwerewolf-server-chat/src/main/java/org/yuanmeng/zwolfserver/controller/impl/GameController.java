...
import org.yuanmeng.zwolfserver.config.Config;
import org.yuanmeng.zwolfserver.controller.BaseController;
import org.yuanmeng.zwolfserver.metadata.business.*;
import org.yuanmeng.zwolfserver.metadata.business.role.Guard;
import org.yuanmeng.zwolfserver.metadata.business.role.Prophet;
import org.yuanmeng.zwolfserver.metadata.business.role.UserRole;
import org.yuanmeng.zwolfserver.metadata.business.role.Witch;
import org.yuanmeng.zwolfserver.metadata.business.ProtocolField;
import org.yuanmeng.zwolfserver.server.manager.IMBusinessManager;
import org.yuanmeng.zwolfserver.service.GameService;
import org.yuanmeng.zwolfserver.service.UserService;
import org.yuanmeng.zwolfserver.util.ByteBufUtil;
...
public class GameController implements BaseController {
    ...
    public void doAccept(short commandId, Channel channel, ByteBuf body, Map<String, Object> application) {
        ...
        int fromPos = room.getPlayers().get(fromId).getPosition();
        switch (commandId) {
            case ProtocolField.CID_GAME_READY_REQ:  //准备、离开游戏

                int ready = gameService.readyGame(fromId, roomId);
                ResponseBody readyBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_READY_RESP,
                        fromId, ready);
                ...
                break;
            case ProtocolField.CID_GAME_START_REQ: //开始游戏
                room = gameService.checkAndGetRoom(roomId, fromId);
                if (room == null) {    //房间为空
                    responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_START_FAIL,
                    ...
                    roleMap.forEach((id, ur) -> {
                        ...
                        ResponseBody resBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_START_RESP,
                                fromId, ur.getRoleId());
                ...
                break;
            case ProtocolField.CID_GAME_KILL_REQ:   //狼人杀人
                Room wolfRoom = gameService.checkAndGetRoom(roomId);
                if (wolfRoom == null) {
                    return;
                }
                GameInfo killInfo = gameService.getGameInfo(roomId);
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_KILL_RESP,
                ...
                timer.schedule(new TimerTask() {
                    ...
                    public void run() {
                        ...
                        //向女巫告知杀人信息
                        ResponseBody killBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_NOTIFY_WITCH_KILLED,
                                0, killed);
                ...
                break;
            case ProtocolField.CID_GAME_SAVE_REQ:  //女巫救人请求
                ...
                break;
            case ProtocolField.CID_GAME_POISON_REQ:  //女巫毒人请求
                ...
                break;
            case ProtocolField.CID_GAME_VERIFY_REQ:  //预言家验人
                ...
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_VERIFY_RESP,
                        code, isGoodReply);
                userChannel = roomChannels.get(fromId);
                msgGourp.put(userChannel, responseBody);
                IMBusinessManager.sendGroup(msgGourp);
                break;
            case ProtocolField.CID_GAME_GUARD_REQ:   //守卫守人
                ...
                break;
            case ProtocolField.CID_GAME_ASK_CHIEF: //请求竞选警长
                if (code == 0) {   //表示不竞选警长
                    break;
                }
                ...
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_ASK_CHIEF_RESP,
                        fromId, code);
                ...
                break;
            case ProtocolField.CID_GAME_QUIT_POLICE: //取消竞选警长
                ...
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_QUIT_POLICE_RESP,
                        fromId, fromId);
                roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                IMBusinessManager.sendGroup(msgGourp);
                break;
//            case ProtocolField.CID_GAME_POLICE_SPEAKING_END:  //警上发言结束
//                gameInfo = gameService.getGameInfo(roomId);
//                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_START_CHIEF_VOTE,
//                        fromId, fromId);
...
//                }
//                break;
            case ProtocolField.CID_GAME_CHIEF_VOTE:  //警上投票
                gameInfo = gameService.getGameInfo(roomId);
                gameInfo.setVoteDetails(0, fromPos, code);  //警上投票bout为0
                gameInfo.putVoteInfo(code);
                doVoteTimer(gameInfo, Config.TIME_GAME_VOTE, Config.GAME_TYPE_CHIEF, roomChannels, room);

                break;
            case ProtocolField.CID_GAME_CHIEF_DECIDE_SPEAK: //警长决定发言顺序
                int actionPos;
                //上一夜死亡名单
                ...
                break;
//            case ProtocolField.CID_GAME_REQ_VOTE:  //请求投票
...
//                }
//                if (chief > 0) {
//                    responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_CHIEF_REQ_SUM_TICKET,
//                            0, 0);
...
//                    IMBusinessManager.sendGroup(msgGourp);
//                } else {
//                    responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_REQ_VOTE_RESP,
//                            0, 0);
//                    roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
//                    IMBusinessManager.sendGroup(msgGourp);
//                }
//                break;
            case ProtocolField.CID_GAME_CHIEF_SUM_TICKET://警长归票
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_REQ_VOTE_RESP,
                        0, code);
                roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                IMBusinessManager.sendGroup(msgGourp);
                ...
                break;
            case ProtocolField.CID_GAME_VOTE:  //投票
                gameInfo = gameService.getGameInfo(roomId);
                gameInfo.setVoteDetails(bout, fromPos, code);
                double mark = GameInfo.DEFAULT_CHIEF_MARK;
                if (fromPos == room.getChief()) {
                    mark = GameInfo.DEFAULT_CHIEF_MARK;
                }
                gameInfo.putVoteInfo(code, mark);
                doVoteTimer(gameInfo, Config.TIME_GAME_VOTE, Config.GAME_TYPE_COMMON, roomChannels, room);
                break;
            case ProtocolField.CID_GAME_REQ_DARK://要求进入天黑
                ...
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_DARK,
                        0, 0);
                roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                IMBusinessManager.sendGroup(msgGourp);
                ...
                doDawnTimer(roomId, bout, Config.TIME_GAME_DARK);
                break;
            case ProtocolField.CID_GAME_CHANGE_CHIEF: //移交警徽
                room.setChief(code);
                responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_CHANGE_CHIEF_RESP,
                        fromId, code);
                roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                IMBusinessManager.sendGroup(msgGourp);
                break;
            case ProtocolField.CID_GAME_TURN_SPEAK_END: //切换聊天者
    ...
    private void doSpeakTimer(Room room, int bout, Map<Integer, UserChannel> roomChannels, long time) {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                ...
                //开始发言
                ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_TURN_SPEAK,
                        bout, speakPos);
                roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                IMBusinessManager.sendGroup(msgGourp);
            }
    ...
    private void speakEnd(int type, Room room, Map<Integer, UserChannel> roomChannels) {
        ...
        if (type == Config.GAME_TYPE_CHIEF) {  //竞选警长发言结束，未上警玩家投票
            ...
            if (chiefPos > 0) {
                ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_CHIEF_REQ_SUM_TICKET,
                        0, 0);
                ...
                ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_REQ_VOTE_RESP,
                        0, 0);
                roomChannels.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                IMBusinessManager.sendGroup(msgGourp);
            }
    ...
    private void askVote4Chief(Room room, Map<Integer, UserChannel> roomChannels) {
        ...
        ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_START_CHIEF_VOTE,
                0, 0);
        List<Integer> chiefList = gameInfo.getChiefVotes();
        gameInfo.addSpeakEndNum();
        roomChannels.forEach((id, chan) -> {
        ...
        timer.schedule(new TimerTask() {
            ...
            public void run() {
                if (type == Config.GAME_TYPE_CHIEF) {
                    ...
                    ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_ELECT_CHIEF_RESP, 0, actionPos);
                    channelMap.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                    IMBusinessManager.sendVoteMsg(msgGourp, gameInfo.getVoteDetails(0), 0);
                    ...
                    ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_VOTE_RESP, 0, votePos);
                    channelMap.forEach((id, chan) -> msgGourp.put(chan, responseBody));
                    int isOver = GameUtil.isGameOver(room.getPlayers());
                    IMBusinessManager.sendVoteMsg(msgGourp, gameInfo.getVoteDetails(bout), isOver);
    ...
    private void doDawn(int roomId, int bout) {
        ...
        Map<String, Object> param = new HashMap<>();
        ResponseBody responseBody = new ResponseBody(ProtocolField.SID_GAME, ProtocolField.CID_GAME_DAWN,
                0, over);
